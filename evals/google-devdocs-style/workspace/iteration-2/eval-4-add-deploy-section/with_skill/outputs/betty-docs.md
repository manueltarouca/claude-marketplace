...
Betty deploys to itch.io through the `butler` CLI.

Before you deploy, set a `BUTLER_API_KEY` secret in the repository.

To deploy, push a tag. CI runs `butler` for you.

To deploy by hand instead, build the game, then run `butler push dist matm/betty:web`.
