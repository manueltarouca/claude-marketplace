...
export const ERRORS = {
  ...
  atlasTooBig: (width: number, height: number) =>
    `The packed atlas is ${width}x${height}, larger than the 4096x4096 maximum texture size. Remove some sprites, shrink the oversized ones, or split the atlas into more pages.`,

  badChannel: (channel: string) =>
    `Unknown upload channel ${channel}. Pass --channel with one of web, windows, or mac.`,
};
