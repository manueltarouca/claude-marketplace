...
export const ERRORS = {
  ...
  atlasTooBig: (width: number, height: number) =>
    `The packed atlas is ${width}x${height}, past the maximum texture size of 4096x4096. Remove sprites from the atlas, shrink the source images, or split them across several atlases.`,

  badChannel: (name: string) =>
    `Upload channel ${name} isn't a known channel. Pass --channel with web, windows, or mac.`,
};
